...
class ImageSearch:
    ...
    def add_to_index(self, image_path):
        print(image_path)
        original_path = Path(image_path).resolve() 
        image_path_relative = original_path.relative_to(settings.BASE_DIR)

        processed_path = original_path.with_name(f"{original_path.stem}_processed{original_path.suffix}")

        with open(image_path_relative, 'rb') as i:
            with open(processed_path, 'wb') as o:
                input = i.read()
                output = remove(input)
                o.write(output)
        feature = self.extract_features(processed_path)
